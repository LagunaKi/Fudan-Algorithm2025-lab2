...
def main():
    ...
    parser.add_argument('--max_gap', type=int, default=30, help='gap区间最大合并距离')
    parser.add_argument('--alpha', type=float, default=64.84, help='query gap惩罚系数')
    parser.add_argument('--gamma', type=float, default=0.776, help='斜率惩罚系数')
    parser.add_argument('--bonus', type=float, default=23.888, help='顺滑延申奖励')
    parser.add_argument('--slope_eps', type=float, default=0.1923, help='主链合并斜率偏差阈值')
    parser.add_argument('--min_k', type=int, default=8, help='最小kmer长度')
    parser.add_argument('--k_ratio', type=float, default=0.90187, help='kmer递减比例')
    parser.add_argument('--ex_max_edit_ratio', type=float, default=0.114, help='主链区间延伸允许的最大edit distance比例')
